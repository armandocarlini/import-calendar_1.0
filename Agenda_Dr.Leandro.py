...
def buscar_agendamentos():
    ...
    while True:
        ...
        resp = requests.get(f"{BASE_URL}/appoints/search", headers=HEADERS, params=params)
        if resp.status_code != 200:
...
def buscar_nome_paciente(paciente_id):
    ...
    resp = requests.get(
        ...
        headers=HEADERS,
        params={"paciente_id": paciente_id, "programa_saude": 1, "photo": 0}
    )
    nome = "Paciente"
    if resp.status_code == 200:
        data = resp.json()
    ...
    for ag in agendamentos:
        try:
            ...
            if not data or not horario:
                continue
            inicio_dt = datetime.strptime(f"{data} {horario}", "%d-%m-%Y %H:%M:%S")
            ...
            evento = {
                "summary": titulo,
                "description": f"Paciente: {paciente_nome}\nMotivo: {motivo}\nStatus: {status_nome}\nConcierge: {assistente}",
                "start": {"dateTime": inicio_dt.isoformat(), "timeZone": TIMEZONE},
                "end": {"dateTime": fim_dt.isoformat(), "timeZone": TIMEZONE},
                "transparency": transparency,
                "extendedProperties": {"private": {"feegow_id": str(feegow_id)}}
            }
            if colorId:
